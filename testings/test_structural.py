...
def test_can_call_existing_endpoints_of_the_API():
    response = requests.get(
        f"{BASE_URL}/get_coordinates", params={"query": "Lima,Peru"}
    )
...
def test_the_result_is_correct_for_all_inputs():
    ...
    for i in range(len(cases)):
        response = requests.get(
            f"{BASE_URL}/get_coordinates", params={"query": cases[i]}
        )
        data = response.json()
...
def test_result_not_found_results():
    response = requests.get(
        f"{BASE_URL}/get_coordinates", params={"query": "abcdefghijklmnoqrstuvwxyz"}
    )
    data = response.json()
